...
def InitDb():
    ...
    query = Database.DbSession.query(ReleaseInfo).filter(
        or_(ReleaseInfo.JobRunningState == JobRunningState.WaitingForStart,
            ReleaseInfo.JobRunningState == JobRunningState.Scheduled,
            ReleaseInfo.JobRunningState == JobRunningState.InProgress))
