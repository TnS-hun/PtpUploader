...
class Ptp:
    ...
    def __LoginInternal():
        ...
        postData = {"username": Settings.PtpUserName, "password": Settings.PtpPassword, "passkey": passKey,
                    "keeplogged": "1"}
    ...
    def GetMoviePageOnPtpByImdbId(logger, imdbId):
        ...
        result = MyGlobals.session.get(
            "http://passthepopcorn.me/torrents.php?imdb=%s&json=1" % Ptp.NormalizeImdbIdForPtp(imdbId))
        response = result.text
        Ptp.CheckIfLoggedInFromResponse(result, response)
    ...
    def UploadMovie(logger, releaseInfo, torrentPath, releaseDescription):
        ...
        # We always use HTTPS for uploading because if "Force HTTPS" is enabled in
        # the profile then the HTTP upload is not working.
        if releaseInfo.HasPtpId():
            logger.info(
                "Uploading torrent '%s' to PTP as a new format for 'http://passthepopcorn.me/torrents.php?id=%s'." % (
    ...
    def SendPrivateMessage(userId, subject, message):
        ...
        # Send the message.
        # We always use HTTPS for sending message because if "Force HTTPS" is enabled in the profile
        # then the HTTP message sending is not working.
