...
class AnnouncementWatcher:
    ...
    def __SetScheduling(releaseInfo):
        ...
        # Cooperation.
        if releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberCount > 1 and 0 <= releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberId < releaseInfo.AnnouncementSource.AutomaticJobCooperationMemberCount and releaseInfo.AnnouncementSource.AutomaticJobCooperationDelay > 0 and len(releaseInfo.ReleaseName) > 0:
    ...
    def __ProcessAnnouncementFileInternal(announcementFilePath, announcementFilename):
        ...
        if announcementId == "0":
            ...
            if len(announcementId) <= 0:
                ...
                time.sleep(3)
                announcementId = AnnouncementWatcher.__TryGettingIdFromContents(announcementFilePath,
                                                                                announcementSource)
                if len(announcementId) <= 0:
    ...
    def LoadAnnouncementFilesIntoTheDatabase():
        ...
        for entry in entries:
            # We can't do anything with undecodable filenames because we can't even join the paths
            # (to move the file to the invalid directory) without getting an UnicodeDecodeError...
