...
class ImageMagick:
    ...
    def OptimizePng(logger, sourceImagePath):
        ...
        if os.path.exists(outputImagePath):
            raise PtpUploaderException("Can't optimize PNG because output file '%s' already exists." % outputImagePath)

        args = [Settings.ImageMagickConvertPath, sourceImagePath, "-quality", "99", outputImagePath]
