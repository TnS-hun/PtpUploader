...
class Mplayer:
    ...
    # The noautosub parameter is not present in all version of MPlayer,
    # and it returns with error code if it is specified.
    @staticmethod
    ...
    # We could get this info when making the first screenshot,
    # but ScaleSize is not stored in the database and screenshots are not made again when resuming a job.
    ...
    def MakeScreenshotInPng(self, timeInSeconds, outputDirectory):
        ...
        # outdir is not working with the Windows version of MPlayer, so for the sake of easier testing we set the
        # output directory with by setting the current working directory.
        # -vf scale=0:0 -- use display aspect ratio
        time = str(int(timeInSeconds))
        args = [Settings.MplayerPath, "-ss", time, "-vo", "png:z=9", "-frames", "1", "-vf", "scale=0:0", "-nosound",
                "-nosub", "-nolirc"]
        if Mplayer.NoAutoSubParameterSupported:
            args.append("-noautosub")
        args.append(self.InputVideoPath)
