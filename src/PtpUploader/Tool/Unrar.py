...
# - .part01.rar
class Unrar:
    # Because there is no easy way to tell if there will be an overwrite upon extraction, we make a temporary directory
    # into the destination path then move the extracted files out of there.
    @staticmethod
    def Extract(rarPath, destinationPath):
        ...
        for file in files:
            ...
            if os.path.exists(destinationFilePath):
                raise PtpUploaderException("Can't move file '%s' to '%s' because destination already exists." % (
