...
class Rtorrent:
    ...
    def AddTorrent(self, logger, torrentPath, downloadPath):
        ...
        # If load_raw is slow then set_directory_base throws an exception
        # (Fault: <Fault -501: 'Could not find info-hash.'>),
    ...
    def AddTorrentSkipHashCheck(self, logger, torrentPath, downloadPath):
        ...
        if os.path.exists(destinationTorrentPath):
            raise PtpUploaderException(
                "Can't create fast resume torrent because path '%s' already exists." % destinationTorrentPath)
        ...
        try:
            ...
            # This ensures that even if adding to rTorrent fails, then resuming the job will work.
            os.remove(destinationTorrentPath)

        return infoHash

    def IsTorrentFinished(self, logger, infoHash):
        # TODO: this try catch block is here because xmlrpclib throws an exception when it timeouts or when the torrent
        # with the given info hash doesn't exists.
        # The latter error most likely will cause stuck downloads so we should add some logic here to cancel an upload.
        # For example: if it haven't download a single byte in ten minutes we can cancel it.
        ...
        try:
            ...
            logger.exception(
                "Got exception while trying to check torrent's completion status. Info hash: '%s'." % infoHash)
    ...
    # rTorrent can't download torrents with fast resume information in them,
    # so we clean the torrents before starting the download.
