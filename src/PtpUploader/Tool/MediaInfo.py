...
class MediaInfo:
    # removePathFromCompleteName: this part will be removed from the path listed at "Complete Name".
    # If removePathFromCompleteName is empty then it will be left as it is.
    ...
    def __ReadMediaInfo(self, logger):
        ...
        # MediaInfo is buggy on some videos and takes a lot of time to finish.
        # We limit this time to the user specified maximum.
        ...
        if thread.isAlive():
            try:
                ...
                raise PtpUploaderException(
                    "Execution of MediaInfo command '%s' failed to finish in 60 seconds." % self.MediaInfoArgs)
        ...
        if self.MediaInfoProcess.returncode != 0:
            raise PtpUploaderException("Execution of MediaInfo command '%s' returned with error code '%s'." % (
    ...
    def __ParseMediaInfo(self, logger):
        # We can't simply store mediaInfoText in self.FormattedMediaInfo
        # because the "Complete name" property gets modified.
            ...
            if line.find(":") == -1:
                ...
                if section == "General":
                    if mediaPropertyName == "Complete name":
                        line = originalMediaPropertyName + separator + self.__MakeCompleteNameRelative(
                            mediaPropertyValue)
    ...
    def IsX264(self):
        return self.Codec == "x264" or (
            (self.Codec == "v_mpeg4/iso/avc" or self.Codec == "avc1") and self.VideoWritingLibrary.find(
                "x264 core") == 0)

    def IsH264(self):
        return (self.Codec == "v_mpeg4/iso/avc" or self.Codec == "avc1") and self.VideoWritingLibrary.find(
            "x264 core") == -1
