...
class ReleaseExtractorInternal:
    def __init__(self, sourcePath, relativeSourcePath, destinationPath, includedFileList,
                 topLevelDirectoriesToIgnore=[], handleSceneFolders=False):
        self.SourcePath = sourcePath
        self.RelativeSourcePath = relativeSourcePath  # Must be separated with "/".
        self.DestinationPath = destinationPath
        self.IncludedFileList = includedFileList
        # Used only for PTP directory when extracting inplace (for File source).
            ...
            if os.path.exists(self.DestinationPath):
                if not os.path.isdir(self.DestinationPath):
                    raise PtpUploaderException(
                        "Can't make destination directory '%s' because path already exists." % self.DestinationPath)
            else:
    ...
    def __HandleDirectory(self, entryName, entryPath):
        ...
        if self.HandleSceneFolders and (fnmatch.fnmatch(entryLower,
                                                        "cd*") or entryLower == "sub" or entryLower == "subs" or entryLower == "subtitle" or entryLower == "subtitles"):
            # Special scene folders in the root will be extracted without
            # making a directory for them in the destination.
            releaseExtractor = ReleaseExtractorInternal(entryPath, self.__MakeRelativeSourcePath(entryName),
                                                        self.DestinationPath, self.IncludedFileList)
            ...
            releaseExtractor = ReleaseExtractorInternal(entryPath, self.__MakeRelativeSourcePath(entryName),
                                                        destinationDirectoryPath, self.IncludedFileList)
            releaseExtractor.Extract()
    ...
    def __HandleFile(self, entryName, entryPath):
        ...
        if os.path.exists(destinationFilePath):
            raise PtpUploaderException("Can't make link from file '%s' to '%s' because destination already exists." % (
...
class ReleaseExtractor:
    @staticmethod
    def __ValidateDirectoryInternal(logger, path, baseRelativePath, includedFileList, throwExceptionForUnsupportedFiles,
                                    videos, additionalFiles):
        ...
        for entry in entries:
            ...
            if os.path.isdir(absolutePath):
                ReleaseExtractor.__ValidateDirectoryInternal(logger, absolutePath, relativePath, includedFileList,
                ...
                # Here we are checking the IncludedFileList for included files
                # because with that it is possible to include unsupported files.
    ...
    def ValidateDirectory(logger, path, includedFileList, throwExceptionForUnsupportedFiles=True):
        ...
        videos = []
        additionalFiles = []
        ReleaseExtractor.__ValidateDirectoryInternal(logger, path, u"", includedFileList,
                                                     throwExceptionForUnsupportedFiles, videos, additionalFiles)
    ...
    def Extract(logger, sourcePath, destinationPath, includedFileList, topLevelDirectoriesToIgnore=[]):
        ...
        releaseExtractor = ReleaseExtractorInternal(sourcePath, u"", destinationPath, includedFileList,
                                                    topLevelDirectoriesToIgnore, handleSceneFolders=True)
        releaseExtractor.Extract()

        # Extract and delete RARs at the destination directory.
        # Subtitles in scene releases usually are compressed twice. Yup, it is stupid.
