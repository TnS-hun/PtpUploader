...
def ReleaseInfoToJobsPageData(releaseInfo, entry):
    ...
    if releaseInfo.HasPtpId():
        if releaseInfo.HasPtpTorrentId():
            entry["PtpUrl"] = "https://passthepopcorn.me/torrents.php?id=%s&torrentid=%s" % (
                releaseInfo.GetPtpId(), releaseInfo.GetPtpTorrentId())
        else:
    ...
    entry["Date"] = TimeDifferenceToText(
        datetime.datetime.utcnow() - Database.TimeStampToUtcDateTime(releaseInfo.LastModificationTime), 2)
