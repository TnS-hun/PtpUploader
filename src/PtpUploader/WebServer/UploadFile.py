...
def ajaxGetDirectoryList():
    ...
    try:
        ...
        for directory in directories:
            ...
            response.append(
                '<li class="directory collapsed"><a href="#" rel="%s/">%s</a></li>' % (currentPath, fileName))

        for file in files:
            currentPath, fileName = file
            extension = os.path.splitext(fileName)[1][1:]  # get .ext and remove dot
            response.append(
                '<li class="file ext_%s"><a href="#" rel="%s">%s</a></li>' % (extension, currentPath, fileName))
    except Exception, e:
...
def ajaxGetInfoForFileUpload():
    ...
    # file is not None even there is no file specified, but checking file as a boolean is OK.
    # (As shown in the Flask example.)
    if (not path):
