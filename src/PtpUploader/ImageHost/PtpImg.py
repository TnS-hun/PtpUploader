    ...
    def __UploadInternal(logger, imagePath=None, imageUrl=None):
        ...
        if imagePath is None:  # Rehost image from url.
            ...
            datagen, headers = poster.encode.multipart_encode(
                [poster.encode.MultipartParam.from_file("uploadfile", imagePath)])
            headers.update(PtpImg.RequiredHttpHeader)
            request = urllib2.Request(
                "http://ptpimg.me/index.php?type=uploadv3&key=QT5LGz7ktGFVZpfFArVHCpEvDcC3qrUZrf0kP", datagen, headers)
