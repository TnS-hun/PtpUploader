...
class SourceBase:
    ...
    def LoadSettings(self, settings):
        ...
        self.AutomaticJobCooperationMemberCount = int(
            settings.GetDefault(self.NameInSettings, "AutomaticJobCooperationMemberCount", "0"))
        self.AutomaticJobCooperationMemberId = int(
            settings.GetDefault(self.NameInSettings, "AutomaticJobCooperationMemberId", "0"))
        self.AutomaticJobCooperationDelay = int(
            settings.GetDefault(self.NameInSettings, "AutomaticJobCooperationDelay", "0"))

        self.StopAutomaticJob = settings.GetDefault(self.NameInSettings, "StopAutomaticJob", "").lower()
        self.StopAutomaticJobIfThereAreMultipleVideos = settings.GetDefault(self.NameInSettings,
    ...
    def ValidateExtractedRelease(self, releaseInfo, includedFileList):
        videoFiles, additionalFiles = ReleaseExtractor.ValidateDirectory(releaseInfo.Logger,
                                                                         releaseInfo.GetReleaseUploadPath(),
                                                                         includedFileList)
        if len(videoFiles) < 1:
            raise PtpUploaderException(
                "Upload path '%s' doesn't contain any video files." % releaseInfo.GetReleaseUploadPath())
    ...
    def Delete(self, releaseInfo, torrentClient, deleteSourceData, deleteUploadData):
        ...
        # (This is needed because of the releases with the same name.
        # This way deleting the second one won't delete the release directory of the first.)
