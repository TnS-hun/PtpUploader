...
class TorrentShack(SourceBase):
    ...
    def __HandleAutoCreatedJob(self, logger, releaseInfo):
        ...
        # We know the release name from the announcement,
        # so we can filter it without downloading anything (yet) from the source.
        releaseNameParser = ReleaseNameParser(releaseInfo.ReleaseName)
        ...
        if releaseName != releaseInfo.ReleaseName:
            raise PtpUploaderException(
                "Announcement release name '%s' and release name '%s' on TorrentShack are different." % (
                    releaseInfo.ReleaseName, releaseName))
    ...
    def DownloadTorrent(self, logger, releaseInfo, path):
        ...
        # If a torrent contains multiple NFO files then it is likely that the site also showed the wrong NFO and
        #  we have checked the existence of another movie on PTP.
    ...
    def GetIdFromUrl(self, url):
        result = re.match(r".*torrentshack\.me/torrents.php\?torrentid=(\d+).*", url)
        if result is None:
            return ""
        else:
    ...
    def GetUrlFromId(self, id):
        return "http://torrentshack.me/torrents.php?torrentid=" + id

    def GetIdFromAutodlIrssiUrl(self, url):
        # http://www.torrentshack.me/torrents.php?action=diwnload&id=12345
        result = re.match(r".*torrentshack\.me\/torrents\.php\?action=download\&id=(\d+).*", url)
        if result is None:
            return ""
        else:
            return result.group(1)
