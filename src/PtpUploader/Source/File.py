...
class File(SourceBase):
    ...
    def ExtractRelease(self, logger, releaseInfo, includedFileList):
        if not releaseInfo.SourceIsAFile:
            ...
            ReleaseExtractor.Extract(logger, releaseInfo.GetReleaseDownloadPath(), releaseInfo.GetReleaseUploadPath(),
                                     includedFileList, topLevelDirectoriesToIgnore)
    ...
    def Delete(self, releaseInfo, torrentClient, deleteSourceData, deleteUploadData):
        # We have to make sure to not to delete source if it is single file
        # because no hard link is being made in this case.
        ...
        if deleteUploadData:
            ...
            # If it is a single file then upload path is its parent directory,
            # so it would be unfortunate to delete. (See GetCustomUploadPath.)
            if sourceIsAFile is False and os.path.isdir(releaseInfo.GetReleaseUploadPath()):
