...
class FunFile(SourceBase):
    ...
    def __ReadTorrentPage(self, logger, releaseInfo):
        ...
        matches = re.search(
            r"""<tr><td class="rowhead" >Size<span id="filelist"></span></td><td class="row1" >.+ \((.+bytes)\)""",
            description)
        if matches is None:
    ...
    def __HandleAutoCreatedJob(self, logger, releaseInfo):
        ...
        if releaseName != releaseInfo.ReleaseName:
            raise PtpUploaderException(
                "Announcement release name '%s' and release name '%s' on FunFile are different." % (
                    releaseInfo.ReleaseName, releaseName))
    ...
    def DownloadTorrent(self, logger, releaseInfo, path):
        ...
        # (We could also try read the NFO with the same name as the release or with the same name as the first RAR and
        # reschedule for checking with the correct IMDb id.)
