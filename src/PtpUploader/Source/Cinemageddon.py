...
class Cinemageddon(SourceBase):
    ...
    # Because some of the releases on CG do not contain the full name of the movie,
    # we have to rename them because of the uploading rules on PTP.
    ...
    def GetCustomUploadPath(self, logger, releaseInfo):
        ...
        name = "%s (%s) %s %s" % (
            releaseInfo.InternationalTitle, releaseInfo.Year, releaseInfo.Source, releaseInfo.Codec)
