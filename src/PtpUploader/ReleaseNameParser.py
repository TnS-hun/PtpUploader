...
class ReleaseNameParser:
    def __init__(self, name):
        ...
        if name.rfind('-') == -1 or name.endswith("-"):
            ...
            if self.__HandleSpecialGroupName(["d", "z0n3"]):
                ...
                pass
                    ...
                    " h 264"):  # For release names without group names. E.g.: Critters.1986.720p.WEB-DL.AAC2.0.H.264
                pass
            else:
        ...
        # This is not perfect (eg.: The Legend of 1900),
        # but it doesn't matter if the real year will be included in the tags.
    ...
    def GetSourceAndFormat(self, releaseInfo):
        ...
        elif self.Tags.IsContainsTag("avc") or self.Tags.IsContainsTag("h264") or self.Tags.IsContainsTags(
                ["h", "264"]):
        ...
        elif self.Tags.IsContainsTag("bdrip") or self.Tags.IsContainsTag("bluray") or self.Tags.IsContainsTags(
                ["blu", "ray"]):
        ...
        if releaseInfo.IsResolutionTypeSet():
            releaseInfo.Logger.info(
                "Resolution type '%s' is already set, not getting from release name." % releaseInfo.ResolutionType)
    ...
    def IsAllowed(self):
        ...
        if len(self.TagsAfterYear.List) > 0:
            match = ReleaseNameParser.__IsTagListContainAnythingFromListOfTagList(self.TagsAfterYear,
                                                                                  Settings.IgnoreReleaseTagAfterYear)
            if match is not None:
                return "'%s' is on your ignore list." % match
        else:
            match = ReleaseNameParser.__IsTagListContainAnythingFromListOfTagList(self.Tags,
                                                                                  Settings.IgnoreReleaseTagAfterYear)
            if match is not None:
                return "'%s' is on your ignore list." % match
