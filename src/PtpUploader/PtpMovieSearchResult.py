...
def GetSourceScore(source):
    scores = {
        ...
        "R5": 6,
        "RC": 6,
        "RC Blu-ray": 6,  # DVD has the same score as HD-DVD and Blu-ray because it must be
...
# - We treat H.264 and x264 equally because of the uploading rules:
# "MP4 can only be trumped by MKV if the use of that container causes problems with video or audio".
# - We treat XviD and DivX equally because of the uploading rules:
# "DivX may be trumped by XviD, if the latter improves on the quality of the former. In cases where the DivX is well distributed and the XviD offers no significant improvement in quality, the staff may decide to keep the former in order to preserve the availability of the movie."
# - We support the checking of possible co-existence for different sized SD XviDs.
# (E.g.: an 1400 MB upload won't be treated as a duplicate of a 700 MB release.)
...
class PtpMovieSearchResult:
    ...
    def __ParseMoviePageMakeItems(self, itemList, torrent):
        ...
        itemList.append(
            PtpMovieSearchResultItem(torrentId, fullTitle, codec, container, source, resolution, remasterTitle, size,
                                     uploadTime))
    ...
    def __ParseMoviePage(self, moviePageJsonText):
        ...
        if len(torrents) <= 0:
            raise PtpUploaderException(
                "No torrents on movie page 'https://passthepopcorn.me/torrents.php?id=%s'." % self.PtpId)
    ...
    def __IsHdFineSourceReleaseExists(self, releaseInfo):
        if releaseInfo.IsRemux():
            if (releaseInfo.Source == "Blu-ray" or releaseInfo.Source == "HD-DVD"):
                return PtpMovieSearchResult.__IsInList(self.HdList, None, ["Blu-ray", "HD-DVD"], ["1080i", "1080p"],
                                                       True)
        else:
    ...
    def __IsHdNonFineSourceReleaseExists(self, releaseInfo, releaseSourceScore):
        if releaseInfo.IsRemux():
            return PtpMovieSearchResult.__IsInListUsingSourceScore(self.HdList, None, releaseSourceScore,
                                                                   ["1080i", "1080p"], True)
        else:
            if releaseInfo.Source == "WEB":
                return PtpMovieSearchResult.__IsInListUsingSourceScore(self.HdList, ["x264", "H.264"],
                                                                       releaseSourceScore, ["720p", "1080p"])
            else:
                if releaseInfo.ResolutionType == "1080p":
                    return PtpMovieSearchResult.__IsInListUsingSourceScore(self.HdList, ["x264", "H.264"],
                                                                           releaseSourceScore, ["1080p"],
                                                                           releaseInfo.IsRemux())
                elif releaseInfo.ResolutionType == "720p":
                    return PtpMovieSearchResult.__IsInListUsingSourceScore(self.HdList, ["x264", "H.264"],
                                                                           releaseSourceScore, ["720p"])
    ...
    def __IsSdFineSourceReleaseExists(self, releaseInfo):
        ...
        if releaseInfo.Source == "Blu-ray" or releaseInfo.Source == "HD-DVD" or releaseInfo.Source == "DVD":
            if releaseInfo.Codec == "x264" or releaseInfo.Codec == "H.264":
                ...
                list = PtpMovieSearchResult.__GetListOfMatches(self.SdList, ["XviD", "DivX"],
                                                               ["Blu-ray", "HD-DVD", "DVD"])
                ...
                if releaseInfo.ResolutionType == "NTSC" or releaseInfo.ResolutionType == "PAL":
                    return PtpMovieSearchResult.__IsInList(self.SdList, [releaseInfo.Codec], ["DVD"],
    ...
    def __IsSdNonFineSourceReleaseExists(self, releaseInfo, releaseSourceScore):
        if releaseInfo.Codec == "DivX" or releaseInfo.Codec == "XviD" or releaseInfo.Codec == "H.264" or releaseInfo.Codec == "x264":
            return PtpMovieSearchResult.__IsInListUsingSourceScore(self.SdList, ["DivX", "XviD", "x264", "H.264"],
                                                                   releaseSourceScore)
...
def UnitTest():
    def MakeTestItem(codec, container, source, resolution, remasterTitle, sizeText):
        return PtpMovieSearchResultItem(0, "", codec, container, source, resolution, remasterTitle,
                                        GetSizeFromText(sizeText), "")
