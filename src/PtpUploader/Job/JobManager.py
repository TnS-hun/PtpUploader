...
class JobManagerItem:
    def __init__(self, releaseInfoId, releaseInfo):
        ...
        # So why this is not in ReleaseInfo?
        # Because SQLAlchemy could possibly return with the same instance when restarting a canceled job.
        # This will be accessed from another thread in case of stop request.
        self.StopRequested = False


# All public methods are thread-safe.
# JobManager must be used from WorkerThread only (except the methods that state otherwise)
# because it deals with ReleaseInfo instances that can't pass thread boundaries because of SQLAlchemy.
# Both PendingAnnouncements and PendingDownloads gets modified from different thread than
# WorkerThread when cancelling a job so we use JobManagerItem.
