...
class CheckAnnouncement(WorkerBase):
    ...
    def __CheckAnnouncementSource(self):
        self.ReleaseInfo.Logger.info(u"Working on announcement from '%s' with id '%s' and name '%s'." % (
            self.ReleaseInfo.AnnouncementSourceName, self.ReleaseInfo.AnnouncementId, self.ReleaseInfo.ReleaseName))
        ...
        if not self.ReleaseInfo.AnnouncementSource.IsEnabled():
            raise PtpUploaderException(
                "Announcement source '%s' is disabled." % self.ReleaseInfo.AnnouncementSourceName)
    ...
    def __CheckSizeLimit(self):
        if (not self.ReleaseInfo.IsUserCreatedJob()) and 0.0 < Settings.SizeLimitForAutoCreatedJobs < self.ReleaseInfo.Size:
    ...
    def __CheckIfExistsOnPtp(self):
        ...
        if self.ReleaseInfo.HasPtpId():
            ...
            # If IMDb ID is not set, then store it,
            # because it is needed for renaming releases coming from Cinemageddon and Cinematik.
        ...
        if existingRelease is not None:
            raise PtpUploaderException(JobRunningState.Ignored_AlreadyExists,
    ...
    def __FillOutDetailsForNewMovieByPtpApi(self):
        ...
        if len(self.ReleaseInfo.Title) > 0:
            self.ReleaseInfo.Logger.info(
                u"Title '%s' is already set, not getting from PTP's movie info." % self.ReleaseInfo.Title)
        else:
        ...
        if len(self.ReleaseInfo.Year) > 0:
            self.ReleaseInfo.Logger.info(
                "Year '%s' is already set, not getting from PTP's movie info." % self.ReleaseInfo.Year)
        else:
        ...
        if len(self.ReleaseInfo.Tags) > 0:
            self.ReleaseInfo.Logger.info(
                "Tags '%s' are already set, not getting from PTP's movie info." % self.ReleaseInfo.Tags)
        else:
        ...
        if self.ReleaseInfo.IsCoverArtUrlSet():
            self.ReleaseInfo.Logger.info(
                "Cover art URL '%s' is already set, not getting from PTP's movie info." % self.ReleaseInfo.CoverArtUrl)
        else:
    ...
    def __FillOutDetailsForNewMovieByExternalSources(self):
        ...
        if self.ReleaseInfo.Title != self.ReleaseInfo.InternationalTitle and len(
                self.ReleaseInfo.InternationalTitle) > 0 and self.ReleaseInfo.Title.find(" AKA ") == -1:
    ...
    def __DownloadTorrentFile(self):
        ...
        self.ReleaseInfo.AnnouncementSource.DownloadTorrent(self.ReleaseInfo.Logger, self.ReleaseInfo,
                                                            sourceTorrentFilePath)

        # Local variable is used temporarily to make sure that SourceTorrentFilePath is only gets stored
        # in the database if DownloadTorrent succeeded.
