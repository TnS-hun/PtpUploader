...
class Upload(WorkerBase):
    ...
    def __StopAutomaticJobIfThereAreMultipleVideosBeforeExtracting(self):
        if self.ReleaseInfo.IsUserCreatedJob() \
                or self.ReleaseInfo.AnnouncementSource.StopAutomaticJobIfThereAreMultipleVideos != "beforeextracting":
            return
    ...
    def __CreateUploadPath(self):
        ...
        customUploadPath = self.ReleaseInfo.AnnouncementSource.GetCustomUploadPath(self.ReleaseInfo.Logger,
                                                                                   self.ReleaseInfo)
    ...
    def __MakeIncludedFileList(self):
        ...
        if len(self.ReleaseInfo.IncludedFiles) > 0:
            self.ReleaseInfo.Logger.info("There are %s files in the file list. Customized: '%s'." % (
    ...
    def __ValidateExtractedRelease(self):
        self.VideoFiles, self.AdditionalFiles = self.ReleaseInfo.AnnouncementSource.ValidateExtractedRelease(
                ...
                if self.ReleaseInfo.IsForceUpload():
                    ...
                    raise PtpUploaderException(
                        "Resolution is set to '%s', detected MediaInfo resolution is '%s' ('%sx%s')." % (
                            self.ReleaseInfo.Resolution, resolution, mediaInfo.Width, mediaInfo.Height))
        else:
    ...
    def __MakeReleaseDescription(self):
        ...
        outputImageDirectory = self.ReleaseInfo.AnnouncementSource.GetTemporaryFolderForImagesAndTorrent(
            self.ReleaseInfo)
        makeScreenshots = not self.ReleaseInfo.IsOverrideScreenshotsSet()
        releaseDescriptionFormatter = ReleaseDescriptionFormatter(self.ReleaseInfo, self.VideoFiles,
                                                                  self.AdditionalFiles, outputImageDirectory,
                                                                  makeScreenshots)
        ...
        # To not waste the uploaded screenshots we commit them to the database
        # because the following function calls can all throw exceptions.
        Database.DbSession.commit()
    ...
    def __MakeTorrent(self):
        ...
        uploadTorrentFilePath = self.ReleaseInfo.AnnouncementSource.GetTemporaryFolderForImagesAndTorrent(
            self.ReleaseInfo)
        uploadTorrentFilePath = os.path.join(uploadTorrentFilePath, uploadTorrentName)

        uploadTorrentCreatePath = ""

        # Make torrent with the parent directory's name included if there is more than one file or
        # requested by the source (it is a scene release).
        totalFileCount = len(self.VideoFiles) + len(self.AdditionalFiles)
        if totalFileCount > 1 or (self.ReleaseInfo.AnnouncementSource.IsSingleFileTorrentNeedsDirectory(
                self.ReleaseInfo) and not self.ReleaseInfo.IsForceDirectorylessSingleFileTorrent()):
        ...
        # Local variables are used temporarily to make sure that values only get stored in the database
        # if MakeTorrent.Make succeeded.
    ...
    def __CheckIfExistsOnPtp(self):
        ...
        # If we are not logged in here that could mean that the download took a lot of time and the user got
    ...
    def __UploadMovie(self):
        ...
        Ptp.UploadMovie(self.ReleaseInfo.Logger, self.ReleaseInfo, self.ReleaseInfo.UploadTorrentFilePath,
        ...
        self.ReleaseInfo.Logger.info("'%s' has been successfully uploaded to PTP. Time taken: %s." % (
    ...
    def __ExecuteCommandOnSuccessfulUpload(self):
        ...
        command = Settings.OnSuccessfulUpload % {"releaseName": self.ReleaseInfo.ReleaseName,
